...
# Ohp Server
clear
echo -e "${cyan}------------[${NC}${multi} Install UDP Custom ${NC}${cyan}]------------${NC}"
sleep 3
...
rm -f /root/ohp.sh
rm -r -f domain
echo -e "[ ${green}Note$NC ] Installing Autoscript Completed.!!"
sleep 2
history -c
cat> /root/.profile << END
# ~/.profile: executed by Bourne-compatible login shells.
...
echo "   - Mod       => Sikecil_Waan :D" | tee -a log-install.txt
echo "======================================================================" | tee -a log-install.txt
echo ""
